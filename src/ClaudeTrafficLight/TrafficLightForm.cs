...
internal sealed class TrafficLightForm : Form
{
    ...
    public TrafficLightForm()
    {
        ...
        _baseDir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "ClaudeLight");
    ...
    {
        _trayMenu.Items.Add("定位 Claude 终端", null, (_, _) => BringClaudeTerminalToFront());
        _trayMenu.Items.Add(new ToolStripSeparator());
        _trayMenu.Items.Add("退出", null, (_, _) => Close());

        _notifyIcon.Text = "Claude 红绿灯";
        _notifyIcon.ContextMenuStrip = _trayMenu;
        _notifyIcon.Visible = true;
        _notifyIcon.DoubleClick += (_, _) => BringClaudeTerminalToFront();
    }
    ...
    private void ReadStatusFile()
    {
        ...
        for (var i = 0; i < 5; i++)
        {
            try
            {
                ...
                _lastUpdate = payload.Time ?? DateTimeOffset.Now;
                SetState(state, payload.CurrentDirectory ?? "", payload.SessionId ?? "");
                return;
            }
            catch (IOException)
            {
    ...
    private static TrafficLightState ParseState(string? state)
    {
        return (state ?? "").Trim().ToLowerInvariant() switch
        {
            ...
            "session_start" => TrafficLightState.Idle,
    ...
    private string GetToolTipText()
    {
        return _state switch
        {
            TrafficLightState.Off => "Claude 未启动或状态过期",
            TrafficLightState.Idle => "Claude 空闲",
            TrafficLightState.Done => "Claude 任务完成",
            TrafficLightState.Thinking => "Claude 正在思考",
            TrafficLightState.Working => "Claude 正在执行任务",
            TrafficLightState.Permission => "等待授权",
            TrafficLightState.Error => "发生错误",
            _ => "Claude 红绿灯"
        };
    }
    ...
    private static void BringClaudeTerminalToFront()
    {
        foreach (var p in Process.GetProcesses())
        {
            try
            {
                ...
                var titleHit = title.Contains("Claude Code - TrafficLight", StringComparison.OrdinalIgnoreCase)
                ...
                if (titleHit || (nameHit && title.Contains("claude", StringComparison.OrdinalIgnoreCase)))
                {
